...
- **Order:** (1) merge/commit, (2) `wrangler deploy`, (3) **for EVERY registered salon** (not just demo — the prune is bucket-wide, so each salon needs a labeled backup first or it loses its recovery point until its next alarm): `POST /state/backup-now` with `X-Salon: <slug>` → key under `backups/<slug>/`, and `GET /state/backups` (`X-Salon: <slug>`) lists only that salon's. (Also force-stamps each salon's `meta:slug`.) (4) only then, `POST /state/prune-legacy` (`{ token: RESTORE_TOKEN }`) → re-run until `pruned:0`, (5) re-verify each salon's list shows its labeled backup and the legacy files are gone.
